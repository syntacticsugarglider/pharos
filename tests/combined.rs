...
fn both() {
    block_on(async move {
        ...
        let mut events: Events<IsisEvent> =
            isis.observe(Channel::Bounded(5).into()).expect("observe");
        let mut nuevts: Events<NutEvent> =
            isis.observe(Channel::Bounded(5).into()).expect("observe");

        let mut ubevts: Events<IsisEvent> =
            isis.observe(ObserveConfig::default()).expect("observe");
