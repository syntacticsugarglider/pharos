...
pub mod import {
    ...
    pub(crate) use {
        futures::{
            channel::mpsc::Receiver, channel::mpsc::UnboundedReceiver, executor::block_on,
            future::poll_fn, sink::Sink, stream::Stream, SinkExt, StreamExt,
        },
        pharos::*,
        std::{
            pin::Pin,
...
impl Observable<IsisEvent> for Goddess {
    ...
    fn observe(
        &mut self,
        options: ObserveConfig<IsisEvent>,
    ) -> Result<Events<IsisEvent>, Self::Error> {
...
impl Observable<NutEvent> for Goddess {
    ...
    fn observe(
        &mut self,
        options: ObserveConfig<NutEvent>,
    ) -> Result<Events<NutEvent>, Self::Error> {
